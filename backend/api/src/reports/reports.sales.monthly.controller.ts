...
export class ReportsSalesMonthlyController {
  ...
  private async tryMonthly(tbl: Tbl, fromMonth?: string, toMonth?: string) {
    ...
    if (!dcols.length || !acols.length) return [];
  ...
  async salesMonthly(@Query('from') from?: string, @Query('to') to?: string) {
    ...
    for (const t of tables) {
      rows = await this.tryMonthly(t, fromMonth, toMonth);
