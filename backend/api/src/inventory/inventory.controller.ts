...
export class InventoryController {
  ...
  async stock(@Query('sku') sku?: string, @Query('take') take = '50') {
    if (sku) {
      const p = await (this.prisma as any).product.findUnique({ where: { sku } });
    ...
    const products = await (this.prisma as any).product.findMany({ take: n, orderBy: { createdAt: 'desc' } });
  ...
  private async getReserved(productId: string) {
    const agg = await (this.prisma as any).stockReservation.aggregate({ where: { productId, status: 'ACTIVA' }, _sum: { qty: true } });
