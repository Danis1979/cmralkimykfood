...
const ORDER_STATUS_VALUES: string[] = [
  'PENDIENTE','CONFIRMADA','CANCELADA','EMITIDA'
]; // TODO: ajustá esta lista a tus estados reales
// TODO: ajustá esta lista a tus estados reales
...
export class OrdersQueryController {
  ...
  async list(
    ...
    if (status) {
      ...
      if (!ORDER_STATUS_VALUES.includes(up)) {
        throw new BadRequestException(
          `status inválido. Usá uno de: ${ORDER_STATUS_VALUES.join(', ')}`
        );
      }
      statusFilter = up;
    }
