...
export class ClientsController {
  ...
  private async tb(): Promise<TableRef> {
    const t = await detectTable(this.prisma, [
      { schema: 'cmr', table: 'Client', nameCols: ['name','nombre'] },
      { schema: 'public', table: 'clients', nameCols: ['name','nombre'] },
      { schema: 'public', table: 'Client', nameCols: ['name','nombre'] },
    ]);
    if (!t) throw new BadRequestException('No hay tabla de clientes (cmr."Client" o public.clients)');
    return t;
  }
  ...
  async search(@Query('page') page='1', @Query('limit') limit='20', @Query('q') q='', @Query('sort') sort?: string) {
    ...
    if (q) {
      // filtra por name/nombre si existe, sino sólo por id::text
      if (t.nameCol) {
        where.push(`("${t.nameCol}" ILIKE '%'||$${pi}||'%' OR CAST(id AS TEXT) ILIKE '%'||$${pi}||'%')`);
      } else {
        where.push(`(CAST(id AS TEXT) ILIKE '%'||$${pi}||'%')`);
      }
      params.push(q); pi++;
    }
    ...
    // total (aprox) – si querés exacto: COUNT(*)
    const totalRows = await this.prisma.$queryRawUnsafe<any[]>(
      `SELECT COUNT(*)::int AS c
         FROM "${t.schema}"."${t.table}"
        ${whereSql}`, ...params
    );
    const total = totalRows[0]?.c ?? items.length;

    return { total, page: p, items };
  }
  ...
  async create(@Body() body: any) {
    ...
    const want: Record<string, any> = {
      name: body.nombre ?? body.name ?? body.client ?? null,
  ...
  async update(@Param('id') id: string, @Body() body: any) {
    ...
    for (const [k,v] of Object.entries(want)) {
      if (v !== null && t.columns.includes(k)) { sets.push(`"${k}" = $${i++}`); vals.push(v); }
    }
